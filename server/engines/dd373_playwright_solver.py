...
def generate_human_steps(distance: float) -> List[Tuple[float, float]]:
    ...
    # 90% distance with easing, 10% overshoot and correction
    main_distance = distance * (1.02 + random.uniform(0.01, 0.03))  # Slight overshoot
...
async def solve_aliyun_slider(page: Page) -> bool:
    ...
    slide_distance = random.uniform(280.0, 320.0)
    ...
    await page.mouse.up()
    time.sleep(2.0)  # Wait for captcha verification response

    # Verify if captcha resolved
    ...
    if "aliyunCaptcha" not in page_content and "Please complete the operation" not in page_content:
        ...
        # Check failure text element
        fail_el = page.locator('#aliyunCaptcha-sliding-failTip').first
        if await fail_el.count() > 0:
            fail_txt = await fail_el.text_content()
            logger.warning(f"[Playwright Solver] Captcha slider fail tip: {fail_txt}")
...
async def fetch_dd373_with_playwright(url: str, max_retries: int = 2) -> Tuple[List[Dict[str, Any]], str]:
    """
    Fetch DD373 page using Playwright stealth persistent browser context.
    Returns (clean_items_list, cookie_header_string).
    """
    start_time = time.time()
    logger.info(f"[Playwright Solver] Launching persistent browser for {url}...")
    ...
    async with async_playwright() as p:
        try:
            ...
            # Check for Captcha & attempt solving
