...
def get_floor_prices(rows, item_name=""):
    if not rows:
        return 0, 0
    ...
    is_sell = "(Sell)" in item_name
    sorted_rows = sorted(rows, key=lambda x: (
        x[1] if x[1] is not None else (-float('inf') if not is_sell else float('inf')), 
        safe_int(x[2])
    ), reverse=not is_sell)
...
async def receive_data(payload: UpdatePayload):
    ...
    raw_floor, trusted_floor = get_floor_prices(temp_rows, payload.item_name)
...
def get_snapshot(item_name: str, hours: float = 24):
    ...
    if old_ts_row and old_ts_row[0]:
        ...
        old_raw, old_trusted = get_floor_prices(old_rows, item_name)
...
def get_history(item_name: str, hours: float = 24):
    ...
    query = f"""
        ...
        ORDER BY timestamp ASC, price {sort_dir}
    """
    ...
    for ts in sorted(grouped.keys()):
        ...
        if ts_val > 0:
            raw, trusted = get_floor_prices(grouped[ts], item_name)
            chart_data.append({
                "time": ts_val,
                "raw_floor": raw,
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Qiandao Backend v1.0 (Port 8003)")
